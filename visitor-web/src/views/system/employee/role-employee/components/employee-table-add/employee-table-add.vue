    ...
    >
      <div>
        ...
        >
            ...
            <Col span="12">
              <FormItem label="登录名" prop="loginName">
                <Input
                  ...
                  :maxlength="30"
                  @on-keydown="
                    formValidate.loginName = formValidate.loginName.replace(
                      /^ +| +$/g,
                      ''
                    )
                  "
            ...
            <Col span="12">
              <FormItem label="姓名" prop="actualName">
                <Input
                  :maxlength="30"
                  @on-keydown="
                    formValidate.actualName = formValidate.actualName.replace(
                      /^ +| +$/g,
                      ''
                    )
                  "
            ...
            <Col span="12">
              <FormItem label="别名">
                <Input
                  :maxlength="30"
                  @on-keyup="
                    formValidate.nickName = formValidate.nickName.replace(
                      /^ +| +$/g,
                      ''
                    )
                  "
            ...
            <Col span="12">
              <FormItem label="身份证">
                <Input
                  ...
                  @on-keyup="
                    formValidate.idCard = formValidate.idCard.replace(
                      /^ +| +$/g,
                      ''
                    )
                  "
          ...
          <Row>
            <Col span="12">
              <FormItem label="手机">
                <Input
                  @on-keyup="
                    formValidate.phone = formValidate.phone.replace(
                      /^ +| +$/g,
                      ''
                    )
                  "
          ...
          <Row>
            <Col span="12">
              <!-- class="select-class"  v-click-outside="clickOutside" -->
              <FormItem
                label="部门"
                required
              >
                <Select v-model="formValidate.departmentId" filterable>
                  <Option
                    :key="item.value"
                    :value="item.value"
          ...
          <Row> 
            <Col span="12">
              <FormItem label="密码" prop="loginPwd" v-if="!formValidate.id">
                <Input
                  @on-keydown="
                    formValidate.loginPwd = formValidate.loginPwd.replace(
                      /^ +| +$/g,
                      ''
                    )
                  "
            ...
            <Col span="12">
              ...
              >
                <Input
                  @on-keydown="
                    formValidate.passwordAgain =
                      formValidate.passwordAgain.replace(/^ +| +$/g, '')
                  "
                  @on-keyup="
                    formValidate.passwordAgain =
                      formValidate.passwordAgain.replace(/^ +| +$/g, '')
          ...
          <Row>
            <Col span="12">
              <FormItem label="排序码">
                  <InputNumber :max="999" :min="0" v-model="formValidate.sort" controls-outside></InputNumber>
              </FormItem>
            </Col>
          </Row>
        </Form>
      ...
      <div slot="footer">
        <Button @click="isShowAddOrUpdateEmployeeModal = false" type="text"
          >取消</Button
        >
        <Button
...
export default {
  ...
  methods: {
    ...
    showModal(row) {
      ...
      //this.formValidate.organizationId = row.id;
      //this.formValidate.organizationName = this.selectDepartment.name;
      //this.formValidate.organizationName = row.name;
      // } else {
    ...
    submitFormData() {
      this.$refs["formValidate"].validate((valid) => {
        if (
          !/^[A-Za-z0-9]{5,17}$/.test(this.formValidate.loginName) &&
    ...
    async getAllDepartments(){
      let result = await departmentApi.getDepartmentAll();
      let datas = result.data;
