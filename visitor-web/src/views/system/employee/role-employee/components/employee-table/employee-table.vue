...
export default {
  ...
  methods: {
    ...
    async getEmployeesByDeptId(){
      let result = await employeeApi.getEmployeeByDeptId(this.departmentId);
      //console.log(result.data);
      //this.totalPage = result.data.total;
      this.employeeTable = result.data;
    },
    // 翻页
    changePage(pageNum) {
    ...
    async sync() {
      try {
        this.$Notice.warning({
            title: '同步',
            desc: '后台正在同步用户、部门信息，这个过程可能需要一到两分钟 '
        });
        let result = await employeeApi.sync();
      } catch (e) {
        console.error(e);
      }finally {
        this.$Notice.success({
