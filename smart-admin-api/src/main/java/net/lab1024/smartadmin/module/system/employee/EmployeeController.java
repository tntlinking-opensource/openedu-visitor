...
public class EmployeeController {
    ...
    public ResponseDTO<List<EmployeeVO>> listEmployeeByDeptId(@PathVariable Long deptId) {
        return employeeService.getEmployeeByDeptId(deptId,null);
    }

    @ApiOperation(value = "通过部门id获取当前部门的人员", notes = "@author yandanyang")
    @GetMapping("/employee/getEmployeeByDeptId/{deptId}")
    public ResponseDTO<List<EmployeeVO>> getEmployeesByDeptId(@PathVariable Long deptId) {
    ...
    @ApiOperation(value = "通过openid获取用户信息 判断当前微信用户是否为教师", notes = "@author wang")
    @GetMapping("/employee/getByOpenid/{openid}")
    @NoNeedLogin
    public ResponseDTO getByOpenid(@PathVariable("openid") String openid) {
        ResponseDTO responseDTO = employeeService.getByOpenid(openid);
        return responseDTO;
    }

    @ApiOperation(value = "用户、部门信息同步", notes = "@author wang")
    @GetMapping("/employee/sync")
    public ResponseDTO sync(){
        return employeeService.sync();
    }
